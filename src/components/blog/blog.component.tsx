export function Blog() {
  return (
    <div className="flex flex-col gap-4 items-center justify-center min-h-screen p-8 pb-20 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <h1 className="text-3xl font-bold">Blog</h1>
      <p className="text-lg">Welcome to the blog page!</p>
      <p className="text-lg">This is a placeholder for blog content.</p>
      <p className="text-lg">More content will be added soon.</p>
      <p className="text-lg">Stay tuned!</p>
      <p className="text-lg">Thank you for visiting!</p>
      <p className="text-lg">Feel free to explore other pages.</p>
      <p className="text-lg">Have a great day!</p>
      <p className="text-lg">Happy blogging!</p>
      <p className="text-lg">Check back for updates!</p>
      <p className="text-lg">We appreciate your support!</p>
      <p className="text-lg">Your feedback is welcome!</p>
      <p className="text-lg">Contact us for inquiries!</p>
    </div>
  );
}
